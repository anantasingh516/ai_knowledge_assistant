...
SECRET_TOKEN = os.getenv("APP_SECRET_TOKEN", "missing_token")
SECURE_HEADERS = {"X-API-Token": SECRET_TOKEN}
...
def generate_excel_sheet(question, answer, citations):
    ...
    if citations:
        for c in citations:
            data.append({
                "Question": question, "Answer": answer, "Citation Source": c['source'], "Chunk ID": c['id'], "Match Score": c['match_score']
            })
    else:
...
with st.sidebar:
    ...
    st.subheader("🎛️ RAG Engine Tuning")
    selected_top_k = st.slider("Retrieval Chunk Depth (Top-K)", min_value=1, max_value=5, value=3)
    selected_temp = st.slider("LLM Temperature (Creativity)", min_value=0.0, max_value=1.0, value=0.2, step=0.1)
    
    st.markdown("---")
    st.subheader("📁 Upload New Documents")
    uploaded_file = st.file_uploader("Drop a file to expand the AI's knowledge base:", type=["txt", "pdf", "docx", "csv"])
    
    if uploaded_file is not None:
        if st.button("🚀 Process & Ingest File", use_container_width=True):
            with st.spinner("Streaming data bytes securely..."):
                try:
                    files = {"file": (uploaded_file.name, uploaded_file.getvalue(), uploaded_file.type)}
                    # Passed SECURE_HEADERS map containing authorization key tokens securely
                    response = requests.post(UPLOAD_URL, files=files, headers=SECURE_HEADERS, timeout=None)
                    if response.status_code == 200:
                        st.success(f"Success! {uploaded_file.name} is now encrypted and indexed.")
                    else:
    ...
    if st.button("🔄 Trigger Manual Re-Index", use_container_width=True, type="primary"):
        with st.spinner("Scanning data vault..."):
            try:
    ...
    st.markdown("---")
    st.subheader("⚖️ Ethical AI Guardrails")
    with st.container(border=True):
        st.caption(
            "**Compliance Notice & Usage Boundaries:**\n"
            "• **PII Protection**: System actively applies local regex scrubbers to mask out emails, phone tracks, and registration keys before caching inputs.\n"
            "• **Deterministic Alignment**: Low temperature constraints keep generation grounded to your document chunks to stop structural hallucinations.\n"
            "• **Educational Utility**: Intended strictly for corporate best practices or course material synthesis. Validate highly sensitive records manually."
        )
                
    st.markdown("---")
    st.caption("AI Assistant Version 1.4 (Secured & Compliant)")
...
with tab1:
    ...
    if user_query := st.chat_input("Ask a question about internal procedures or algorithms..."):
        ...
        with st.chat_message("assistant"):
            ...
            with st.spinner("Searching document vault securely..."):
                try:
                    payload = {
                        "question": user_query, 
                    ...
                    res = requests.post(API_URL, json=payload, headers=SECURE_HEADERS, timeout=None)
    ...
    if st.session_state.last_response:
        ...
        with col1:
            ...
            st.download_button(
                label="📄 Download Report as Word (.docx)", data=doc_bytes,
        ...
        with col2:
            ...
            st.download_button(
                label="📊 Download Table Data as Excel (.xlsx)", data=xls_bytes,
        ...
        if st.button("🚀 Run Evaluation Suite Benchmark", use_container_width=True):
            with st.spinner("Querying vector indexing coordinates programmatically..."):
                try:
                    ...
                    run_automated_evaluation(k=selected_top_k)
